...
public class VeinFeature extends Feature<VeinConfiguration> {
    ...
    public TagKey<Biome> getKey (String name) {
        return TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(name.substring(1)));
    }
            ...
            for (int j = -_radius; j <= _radius; j++) {
                ...
                if (i * i + j * j <= _dr * _dr) {
                    ...
                    while (_top.getY() > -63) {
                        ...
                        if (_blockState.is(BlockTags.OVERWORLD_CARVER_REPLACEABLES) && !(_blockState.getMaterial() == Material.WATER)) {
