...
public class VeinFeature extends Feature<VeinConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<VeinConfiguration> context) {
        ...
        BlockPos at = context.origin();
        Holder<Biome> wb = world.getBiome(at);

        int sumChance = 0;
        for (VeinConfig cfg : Configuration.veinsList) {
            if (wb.is(TagKey.create(ForgeRegistries.BIOMES.getRegistryKey(), new ResourceLocation(cfg.biometag)))) sumChance += cfg.chance;
        }
        ...
        while (cc >= 0) {
            ...
            if (wb.is(TagKey.create(ForgeRegistries.BIOMES.getRegistryKey(), new ResourceLocation(cfg.biometag)))) cc -= cfg.chance;
            if (cc >= 0) nn++;
        }
